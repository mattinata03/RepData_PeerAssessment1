...
Results:
The maximum steps taken at any time (**actMaxPerInt**) is `r actMaxPerInt`.

The 5-minute interval (**actMax5MinTime**) at which these are is `r actMax5MinTime`.
...
The number of missing values in the **steps** field is confirmed to be  `r actNAcount`.
...
We confirm that there are `r cl_actNAcount` empty records in the **steps** field of the new file.
...
The difference is `r diffMean` -- the mean has not changed.
...
The difference is `r diffMedian` -- which is not significant.
